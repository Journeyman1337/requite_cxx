...
void Builder::generate_call_arguments(r::Procedure& callee, const r::Operation& operation, llvm::SmallVector<llvm::Value*>& llvm_arguments)
{
...
void Builder::generate_call_statement(const r::Operation& operation)
{
    ...
    assert(!callee.get_is_constructor());
    assert(!callee.get_is_destructor());
    ...
    this->generate_call_arguments(callee, operation, llvm_arguments);
    this->llvm_builder->
        CreateCall(
