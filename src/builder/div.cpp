...
llvm::Value* Builder::generate_div_group_expression(std::span<const r::Expression> span, const r::Type& expected_type)
{
...
llvm::Value* Builder::generate_div_value_expression(const r::Operation& operation, const r::Type& expected_type)
{
    assert(operation.opcode == r::Opcode::DIVIDE);
    assert(operation.branches.size() >= 2UZ);
    return this->generate_div_group_expression(operation.branches, expected_type);
}

void Builder::generate_div_assignment_statement(const r::Operation& operation)
{
    assert(operation.opcode == r::Opcode::DIVIDE_EQUAL);
    assert(operation.branches.size() >= 2UZ);
    ...
    llvm::Value* llvm_value =
        this->generate_div_group_expression(
            operation.branches,
            store_type
        );
...
llvm::Value* Builder::generate_div_assignment_value_expression(const r::Operation& operation, const r::Type& expected_type)
{
    assert(operation.opcode == r::Opcode::DIVIDE_EQUAL);
    const r::Expression& store_location_expression = operation.branches.front();
    r::Type store_type = this->resolver.deduce_type(store_location_expression, this);
