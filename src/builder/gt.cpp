...
llvm::Value* Builder::generate_compare_gt_value_expression(const r::Operation& operation, const r::Type& expected_type)
{
    assert(operation.opcode == r::Opcode::GREATER);
...
llvm::Value* Builder::generate_compare_gteq_value_expression(const r::Operation& operation, const r::Type& expected_type)
{
    assert(operation.opcode == r::Opcode::GREATER_EQUAL);
    assert(operation.branches.size() >= 2UZ);
    this->resolver.check_type_assignable_to_type(r::BOOL_TYPE, expected_type);
    r::Type argument_type = this->resolver.deduce_group_type(operation.branches, this);
    if (argument_type.get_is_signed_integer())
